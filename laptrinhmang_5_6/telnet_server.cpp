...
void *threadPerClient(void *arg) {
   ...
   while (1) {
      ...
      if (!isLogged) {
         ...
         if (ret != 2) {
            // Wrong format
            const char *message = "Wrong format (acc pass)\n";
         ...
         if (!login(file_db, acc, pass)) {
            // Wrong format
            const char *message = "Wrong account or password\n";
            send(client, message, strlen(message), 0);
            continue;
         }
   ...
   close(client);
   return NULL; // Add a return statement to match the function signature
}
