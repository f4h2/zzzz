...
void *threadPerClient(void *arg) {
   ...
   while (1) {
      ...
      if (!isLogged) {
         ...
         if (ret != 2) {
            // Wrong format
            const char *message = "Wrong format (client_id: client_name)\n";
         ...
         const char *message = "Login successfully\n";
         send(client, message, strlen(message), 0);
         isLogged = true;
      } else {
         // Logged in
         sprintf(sendBuff, "%04d/%02d/%02d %02d:%02d:%02d %s:%s\n",
                 ...
                 tm.tm_min, tm.tm_sec, client_name, buff);
...
int main(int argc, char *argv[]) {
   ...
   while (1) {
      ...
      if (numberOfClients < MAX_USER) {
         ...
         pthread_create(&thread_id, NULL, threadPerClient,
                        (void *)&clientSocket);
         pthread_detach(thread_id);
      } else {
         // Full connection
         printf("Exceed maximum connection\n");
         const char *message = "Exceed maximum connection\n";
